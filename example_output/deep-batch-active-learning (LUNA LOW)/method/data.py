...
def _download_mnist_subset(
    ...
    if os.environ.get("R2C_OFFLINE", "").strip().lower() in {"1", "true", "yes", "on"}:
        ...
        # network download (the 2026-07-13 stage-2d dry-run stall).
        raise RuntimeError(
            "R2C_OFFLINE is set: refusing to download MNIST. Place a "
            ".pt/.json/.csv dataset under method/example_data/ (or pass "
            "`path=`) to run without network access."
        )
    try:
        from torchvision import datasets, transforms
    except ImportError as e:  # actionable, classifiable env error (ENV-4)
        raise ImportError(
            "The MNIST smoke/example fallback needs torchvision (declared in "
            "requirements.txt). Install it (`pip install torchvision`), or place a "
            ".pt/.npz/.csv dataset under method/example_data/ to skip the download."
        ) from e
